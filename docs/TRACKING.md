...
| P5-15 | Visualizer modes: Now Playing / Ambient / Full / auto-enter on idle | ✅ | Wired: `components/Visualiser.svelte` owns a canvas and nothing else, `lib/visualiser.ts` owns every decision and is tested in Node. The panel's **resting appearance is unchanged** and the loop is stopped there — the approved wash stays until the visualiser-as-backdrop question is prototyped on a page (D-076). The machine is ticked from two places, because the resting state is the only one it ever needs to go idle *from* and the render loop is stopped there. The swallowed wake-up touch cancels the `pointerdown` **and** the click it produces: every control acts on click, and whether a cancelled pointer event suppresses it is implementation-defined (D-077). No WebGL2 is a supported panel, not an error |
...
| P5-44 | `fractal` scene — Julia orbit trap, the cover as the trap texture | ✅ | Shipped as the `Orbit` look. The cover is sampled at the position of the orbit point that came closest to the origin, so it enters the picture everywhere the dynamics send an orbit through the trap disc — warped and repeated down into the filaments. With no cover bound there is no picture at all, which is family E's test in its strongest form. Moves by walking the parameter inside the main cardioid rather than zooming: fp32 runs out of mantissa about twenty seconds into a zoom, and the cardioid interior guarantees the set stays connected |
| P5-45 | `terrain` scene — the cover as a heightfield | ✅ | Shipped as the `Relief` look. The vertex stage samples `uArt` directly: a sampler uniform is program-scoped, not stage-scoped, and GLES 3.0 mandates at least 16 vertex texture units where GLES 2.0's minimum was zero. Three things make a 256px sleeve read as land — a five-tap cross low-pass whose four arms are *also* the gradient (so the lighting costs nothing beyond the smoothing), a contrast expansion whose derivative is applied to the gradient too, and height taken from tone rather than luma, since Rec. 601 flattens a saturated blue sleeve to 0.11 everywhere. The cover is **mirrored, not repeated**: a repeat puts a cliff at every tile edge, and a cliff in a heightfield is a wall |
| P5-46 | `starfield` scene — the cover sampled into depth | ✅ | Shipped as the `Drift` look. A 32×32 jittered lattice where each star samples the cover at its own place in it — one star per 8×8 block of a 256px sleeve, scattered through depth and flown through. Colour is the block's hue normalised by its peak channel, so dark blocks give dark stars rather than invisible ones. `density` weights brightness by block luminance rather than spawning, because the generator is pure and has never seen the artwork. Quads, not points: `GlContext.draw` issues only `TRIANGLES`, and a point sprite cannot be stretched into a streak. Beat streaks rather than flashes, dividing colour by `sqrt(stretch)` so the light on the panel is unchanged |
| P5-34 | `Bevel` theme — bevelled chrome + `ambient` scene | ✅ | Renamed off the working title per D-015 — named for what it does to the chrome, not for the product it is an homage to. Near-square corner, no blur, a hard bright top edge and a press with no transition: the whole visual grammar of a raised 1995 control. Pins no palette; the scene is already using the cover twice |
| P5-35 | `ambient` scene — slow geometric solids | ✅ | Shipped as the `Solids` look. **The cover twice over** — the field behind the solids is the cover aspect-cropped and dimmed, and every facet is a centred crop of the same cover, key-lit — which is what a desktop theme of that era actually was: wallpaper *and* screensaver. 100 vertices, 42 triangles, one draw call. Sorts itself twice because the engine cannot: back faces are rejected in the vertex shader from the *face plane* (so all three corners agree and a silhouette face is never half-culled), and solids are ordered far-to-near by `buildAmbientGeometry`, which **throws** if two depth spans could touch at the param ceilings. Beat drives position and scale only (D-071) |
| P5-36 | Theme switching UI + shuffle-on-track-change | ✅ | `VisualList` — the plate grown, like Devices and Queue, reached from the `VISUAL` chip. One tap moves all three halves of a theme: its look, its chrome, and its palette where it pins one. **Shuffle is separate from the theme on purpose** — it changes the *look* every track and leaves chrome and palette alone, because shuffling whole themes repaints the panel and reshapes its corners every three minutes, which is a fault rather than a mode. The chip opens the list rather than going full-screen: the panel gets there on its own after 45s, so what the chip is useful for is choosing what it will show when it does |
